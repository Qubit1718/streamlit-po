...
if st.button('Next'):
    tickers = ticker_input.replace(' ', '').split(',')  # Remove spaces and split by comma
    ...
    if tickers and tickers[0]:  # Check if there's at least one ticker
        ...
        if not close_prices.empty:
            ...
            stock_closing_prices_3y = pd.read_csv('stock_closing_prices_3y.csv')
            closing_prices_reset = close_prices.reset_index(drop=True)
            closing_prices_reset_3y = stock_closing_prices_3y.reset_index(drop=True)
            ...
            for s in log_returns.columns:
                plt.plot(closing_prices_reset_3y[s], label=s)
            legend_fontsize = 8
